...
class CodeClassificationAgent:
    def __init__(self, model_path: str = "/home/francisca/Desktop/mei/LEGISLAI/legislai-be/multi_agent/classifier_nlp/model-best"):
        """
    ...
    def _classify_text(self, text: str) -> List[Dict[str, float]]:
        ...
        return [{'category': cat, 'score': float(score)} 
    ...
    def _get_explanation(self, text: str) -> Dict[str, Any]:
        ...
        def predict_proba(texts):
            ...
            return np.array([[doc.cats.get(cat, 0.0) 
                            for cat, _ in sorted_cats] 
                            for doc in docs])
        
        explainer = LimeTextExplainer(
            class_names=[cat for cat, _ in sorted_cats]
        )
        
        exp = explainer.explain_instance(
            self._remove_stopwords(text),
            predict_proba,
            num_features=10,
            num_samples=3
        )
    ...
    def _remove_stopwords(self, text: str) -> str:
        ...
        """
        doc = self.nlp(text)
        filtered_words = [token.text for token in doc if not token.is_stop and not token.is_punct]  # Filtra stop words e pontuação
